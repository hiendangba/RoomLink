...
const RenewalManagementPage = ({ onSuccess, onCancel }) => {
  const { user } = useAuth();
  ...
  // Load history when filters change (Admin only)
  useEffect(() => {
    if (isAdmin) {
      loadHistory();
    }
  }, [filterStatus, currentPage, searchKeyword, isAdmin]);
  ...
  return (
    <PageLayout
      title={isAdmin ? "Quản lý đợt yêu cầu" : "Đợt yêu cầu phòng ở"}
      subtitle={isAdmin ? "Quản lý các đợt yêu cầu về phòng ở (gia hạn, chuyển phòng, hủy phòng)" : "Xem trạng thái đợt yêu cầu về phòng ở (gia hạn, chuyển phòng, hủy phòng)"}
      showClose={true}
      onClose={onCancel}
    >
          ...
          <LoadingState isLoading={loadingActive} loadingText="Đang tải..." className="py-8">
            {activeRenewal ? (
                ...
                {!isAdmin && (
                  <div className="mt-4 p-4 bg-blue-50 rounded-lg">
                    <p className="text-sm text-blue-800">
                      <strong>Lưu ý:</strong> Khi có đợt yêu cầu đang hoạt động, bạn có thể thực hiện các yêu cầu về phòng ở (gia hạn, chuyển phòng, hủy phòng).
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="text-center py-8">
                <InfoBox
                  type="info"
                  messages={['Hiện tại không có đợt yêu cầu nào đang hoạt động.']}
                />
                {isAdmin && (
                  <div className="mt-4">
                    <Button
                ...
                {!isAdmin && (
                  <div className="mt-4 p-4 bg-yellow-50 rounded-lg">
                    <p className="text-sm text-yellow-800">
                      Vui lòng chờ admin tạo đợt yêu cầu mới để có thể thực hiện các yêu cầu về phòng ở (gia hạn, chuyển phòng, hủy phòng).
                    </p>
                  </div>
                )}
              </div>
